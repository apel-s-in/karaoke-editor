...
_bindPanelDragDrop(){
  document.querySelectorAll('.panel').forEach(p=>{
    ...
    p.addEventListener('dragend',()=>p.classList.remove('dragging-panel'));
  });

  document.querySelectorAll('.dock-zone').forEach(z=>{
    z.addEventListener('dragover',e=>{
      if(this._toolbarLocked || !e.dataTransfer.types.includes('panel-id')) return;
    ...
    z.addEventListener('drop',e=>{
      ...
      const pid = e.dataTransfer.getData('panel-id');
      const p = document.getElementById(pid);
      if(p){ z.appendChild(p); this.applyViewPanels(); this._savePanelsOrder(); }
    });
  });
},
...
_bindToolbarGroupDrag(){
  ...
  this._applyToolbarLock(this._toolbarLocked!==false);
...
_applyToolbarLock(locked){
  ...
  document.body.classList.toggle('tb-locked',locked); // Глобальный класс для CSS
  this.ui.toolbarLockBtn.textContent=locked?'🔒':'🔓';
  this.ui.toolbarLockBtn.classList.toggle('unlocked',!locked);
  this.ui.toolbarLockBtn.title=locked?'Разблокировать перемещение окон и кнопок':'Заблокировать перемещение окон и кнопок';
...
startResize(e, el){
  ...
  e.preventDefault();
  const target=el.dataset.target, dock=document.getElementById(target);
  if(!dock)return;
  el.classList.add('active');
  this.resize={active:true, target, startX:e.clientX, startY:e.clientY, el,
    startDim: target==='dock-bottom' ? dock.offsetHeight : dock.offsetWidth};
  document.body.style.cursor = target==='dock-bottom' ? 'row-resize' : 'col-resize';
},
...
handleResize(e){
  ...
  if(target==='dock-right'){
    dock.style.width=Math.max(200,Math.min(800,startDim-(e.clientX-startX)))+'px';
  }else if(target==='dock-left'){
    dock.style.width=Math.max(200,Math.min(800,startDim+(e.clientX-startX)))+'px';
  }else if(target==='dock-bottom'){
    dock.style.height=Math.max(100,Math.min(600,startDim-(e.clientY-startY)))+'px';
  }
  this.renderRuler();
},
