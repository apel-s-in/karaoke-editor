...
// Высота строки = высота текста + 26px (полоска меню сверху) + 6px зазор
itemH(tr){return Math.round(30*this.verticalZoom*(tr.rowScale||1))},
rowH(tr){return this.itemH(tr)+32},
trackTopFor(tr){
  let top=0;
...
renderTimeline(){
  ...
  this.getVisibleTracks().forEach(tr=>{
    ...
    // Серая зона = только высота текста (lh) + отступ под полоску меню (26px сверху)
    const bgTop=trackTop+26,bgH=lh+4;
...
createTrackItemElement(tr,it,trackTop,lh,color,isActive){
  ...
  div.className=cls;
  const itTop=trackTop+28+(it.kind==='word'?lh*.5:0);
...
finalizeMarquee(){
  ...
  const tTop=this.trackTopFor(tr)+base+28,lh=this.itemH(tr);
  tr.items.forEach(it=>{
