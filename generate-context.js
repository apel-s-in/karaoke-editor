...
const FULL_FILE = path.join(META_DIR, 'project-karaoke-editor-full.txt');
const ADAPTIVE_FILE = path.join(META_DIR, 'project-karaoke-editor.txt');
...
const EXCLUDE_RAW = [
  ...
  'yarn.lock',
  'pnpm-lock.yaml',
  'project-karaoke-editor-full*.txt',
  'project-karaoke-editor-adaptive*.txt'
];
...
const headerBlock = () => {
  ...
  const repoUrl = String(argv['repo-url'] || readRepoUrl() || 'https://github.com/apel-s-in/karaoke-editor');

  return `${rules}Название репозитория: ${repoName}
Адрес репозитория: ${repoUrl}
Назначение: отдельное приложение для редактирования караоке файлов к песням.
